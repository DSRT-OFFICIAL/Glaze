// ==================== index.d.ts ====================

import { GlazeBasicColors } from "./GlazeBasicColors";
import { GlazeExtendedColors } from "./GlazeExtendedColors";
import { GlazeVariantsColors } from "./GlazeVariantsColors";
import { GlazeNamedColors } from "./GlazeNamedColors";
import { GlazeGradients } from "./GlazeGradients";
import * as GlazeUtils from "./GlazeUtils";

export { GlazeBasicColors };
export { GlazeExtendedColors };
export { GlazeVariantsColors };
export { GlazeNamedColors };
export { GlazeGradients };
export { GlazeUtils };
